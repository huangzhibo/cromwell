  ...
  val workflowStoreReceive: Receive = {
    ...
    case cmd: BatchSubmitWorkflows =>
      ...
      futureResponses onComplete {
        case Success(worklfowSubmissionResponses) =>
          log.info("Workflows {} submitted.", worklfowSubmissionResponses.toList.map(res => res.id).mkString(", "))
          val labelsMap = convertJsonToLabelsMap(cmd.sources.head.labelsJson)
          worklfowSubmissionResponses.map(res => publishLabelsToMetadata(res.id, labelsMap))
          sndr ! WorkflowsBatchSubmittedToStore(
            worklfowSubmissionResponses.map(res => res.id),
            convertDatabaseStateToApiState(worklfowSubmissionResponses.head.state)
          )
          removeWork()
        case Failure(throwable) =>
          log.error("Workflow {} submit failed.", throwable)
